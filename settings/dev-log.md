...
- **Health:** Green (Step 31 completed - Bollinger Band Mean Reversion Strategy implemented).
...
- [x] **Step 29: Timeframe Migration (1H → 4H) and 8D Rescaling:** Migrated strategy to 4H timeframe (institutional standard for swing trading) to reduce noise. Implemented max hold period exit logic (8th dimension) to improve capital efficiency. Updated all optimization default ranges for 4H physics (fast: [8,13,21], slow: [21,34,50,89], ADX threshold: [20,25], MACD fast: [12,16], max hold hours: [48,72,96,120]). Backtesting engine now enforces time-based exits secondary to SL/TP. Target OOS Sharpe: ≥ 0.8.
- [x] **Step 30: Symmetry Blockade (Long Only Diagnostic):** Implemented architectural mechanism to disable all SELL (SHORT) signals through configurable `long_only` flag in `StrategyConfig`. When enabled, strategy operates exclusively in long direction, isolating LONG signal performance from failed short trades. Signal blockade applied at strategy level (both `VolatilityAdjustedStrategy` and `SmaCrossStrategy`) using vectorized NumPy operations. Maintains full architectural separation - filters and backtesting engine unchanged. Diagnostic target: OOS Sharpe ≥ 0.50 to validate trend-following approach.
- [x] **Step 31: Strategy Core Pivot - Bollinger Band Mean Reversion:** Implemented new `BollingerBandStrategy` class pivoting from trend-following (EMA Cross) to mean reversion using Bollinger Bands for entry signals. Strategy detects price over-extension: LONG trigger when price crosses below Lower Band (oversold), SHORT trigger when price crosses above Upper Band (overbought). Maintains full compatibility with existing filter architecture (ADX Regime Filter + MACD Momentum Filter), inherits risk management features (ATR/Max Hold), and supports `long_only` mode. All calculations 100% vectorized using pandas/numpy. Integrated into strategy factory and can be instantiated via `run_backtest.py`, `run_live.py`, and `tools/optimize_strategy.py` using strategy name `"BollingerBandStrategy"` or `"bollinger_band"`. Configuration parameters: `bb_window` (default: 20) and `bb_std_dev` (default: 2.0). Validation target: OOS Sharpe ≥ 0.50 to validate mean reversion as viable trading edge.
- [x] **Step 32: WFO Framework Extension for Bollinger Bands:** Extended optimization framework (`tools/optimize_strategy.py`) to accept and construct parameter grid using Bollinger Band parameters (`bb_window`, `bb_std_dev`) instead of EMA/SMA parameters. Framework is now fully strategy-aware, automatically detecting strategy type from config.json and using appropriate parameters. Updated parameter combination generation logic for all dimensions (2D-8D) to support BB strategy. Updated all logging and output formatting to display BB parameters correctly. Updated `tools/analyze_optimization.py` to handle BB parameters in metadata and formatting. JSON output files now correctly contain only the 8 relevant parameters for BB strategy: BB_Window, BB_Std Dev, ATR_Window, ATR_Multiplier, ADX_Window, ADX_Threshold, MACD_Fast, and Max Hold Hours. Default BB optimization ranges: bb_window [14,20,30], bb_std_dev [1.5,2.0,2.5], with adjusted ADX thresholds for mean reversion.
